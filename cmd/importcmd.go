...
var importCmd = &cobra.Command{
	Use:   "import",
	Short: "Import JSONL data files into the SQLite database",
	Long:  "Reads JSONL files from data/nuget/crawl/ and data/nuget/hashes/, merges hashes, and bulk-inserts into SQLite",
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		hashed := make(map[string]db.DLLMatch) // key: dll_name+package_name
		hashFiles, _ := filepath.Glob("data/nuget/hashes/hashed_*.jsonl")
		...
		crawlFiles, _ := filepath.Glob("data/nuget/crawl/page_*.jsonl")
		...
		for _, f := range crawlFiles {
			...
			for _, e := range entries {
				...
				if h, ok := hashed[key]; ok {
					e.Hash = h.Hash
					delete(hashed, key) // mark as consumed
				}
		...
		cursorData, err := os.ReadFile("data/nuget/crawl/cursor.txt")
		if err == nil && len(cursorData) > 0 {
			store.SetCursor(string(cursorData))
		}

		fmt.Printf("Done. Imported %d NuGet entries.\n", total)
		return nil
	},
}
